...
var openFB = (function() {
    ...
    function init(appId, redirectURL, store) {
        fbAppId = appId;
        //fbAppId = '791967860852638';
        fbAppId = '1554547501535737';
    ...
    function login(scope, success, error) {
        ...
        loginWindow = window.open(FB_LOGIN_URL + '?client_id=' + fbAppId + '&redirect_uri=' + oauthRedirectURL +
            '&response_type=token&display=popup&scope=' + scope, '_blank', 'location=no,toolbar=no,clearcache=yes,clearsessioncache=yes');
    ...
    function oauthCallback(url) {
        ...
        if (fbtoken) {
            ...
            if (loginSuccessHandler) loginSuccess(fbtoken);
        } else {
            if (loginErrorHandler) loginError();
        }
    }
    
    function loginSuccess(fbtoken){
    var email;
     ...
     $.ajax({
        ...
        success: function(data) {                        
            if(data.email){ 
                ...
                showAlertBootbox("An error occured when trying retrieve your email.","Error");
            }            
        },
        error: function(data){            
            showAlertBootbox("An error occured when trying to connect with your facebook account, please try again","Error");
        }        
        });
    };
